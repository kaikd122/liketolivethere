...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    const result =
      await prisma.$executeRaw`INSERT INTO "Review" (id, body, title, user_id, town_id, latitude, longitude, geom, updated_at, rating) VALUES (${id}, ${data.body}, ${data.title}, ${data.userId}, ${nearestTown[0].id}, ${data.latitude}, ${data.longitude}, st_setsrid(st_makepoint(${data.longitude},${data.latitude}),4326), current_timestamp, ${data.rating});`;
