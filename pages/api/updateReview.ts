...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    const result =
      await prisma.$executeRaw`UPDATE "Review" SET id = ${id}, body = ${data.body}, last_lived_year = ${data.lastLivedYear}, title = ${data.title}, user_id = ${data.userId}, town_id = ${nearestTown[0].id}, latitude = ${data.latitude}, longitude = ${data.longitude}, geom = st_setsrid(st_makepoint(${data.longitude},${data.latitude}),4326), updated_at = current_timestamp, rating = ${data.rating} WHERE id = ${id};`;
